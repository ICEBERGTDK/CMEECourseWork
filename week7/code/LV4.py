...
"""Script that plots a discrete-time version of the Lotka-Volterra model with a
random gaussian fluctuation in resource’s growth rate at each time-step,
...
def discreteLV(r0, c0, t, r, a, z, e, K, E):
    ...
    Arguments:
     ...
     - E: Random fluctuation drawn from gaussian distribution
...
def main(r=1, a=0.1, z=1.5, e=0.75, K=30, tN=15):
    ...
    E = np.random.normal(0, 1)  # random fluctuation drawn from gaussian dist

    # Run LV function and extract results
    RC = discreteLV(R0, C0, t, r, a, z, e, K, E)
    res = [x[0] for x in RC]
